    ...
    return (
        ...
        {food?.name && (
            <div className='flex items-center gap-2'>
                {food?.image && (
                      <div className='w-3/12 h-24 md:h-full lg:h-36 md:px-3'>
                          <img src={food?.image} alt='food item' className='w-full h-full rounded-lg' />
                      </div>
                )}
                <div className='w-3/4 md:w-7/12 flex flex-col gap-1'>
                    <div className='flex items-center justify-between'>
                        <h3 className='text-xl font-semibold'>{food?.name}</h3>
                        <button onClick={addFoodToCart} disabled={food.isAddedToCart}  className='md:hidden flex items-center gap-2 text-zomato-400 border border-zomato-400 bg-zomato-50 px-2 py-1 rounded-lg'>
                        {food.isAddedToCart ? ("Added") : (<><AiOutlinePlus /> Add</>)} 
                        </button>
                    </div>
                    <ReactStars count={5} value={food?.rating || 0} />
                    <h5>$ {food?.price}</h5>
                    <p className='truncate'>{food?.descrition}</p>
                </div>
                <div className='hidden md:block w-2/12'>
                <button onClick={addFoodToCart} disabled={food.isAddedToCart} className='flex items-center gap-2 text-zomato-400 border border-zomato-400 bg-zomato-50 px-2 py-1 rounded-lg'>
                           {food.isAddedToCart ? ("Added") : (<><AiOutlinePlus /> Add</>)} 
                        </button>
                </div>
            </div>
